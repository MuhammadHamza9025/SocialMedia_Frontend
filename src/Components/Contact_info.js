...
const Contact_info = ({ setconat, contact }) => {
    ...
    return (
        <div className={`h-[100vh] w-[100vw] justify-center items-center fixed top-0 right-0   bg-gray-500 bg-opacity-20 ${contact ? 'flex' : 'hidden'}`}>
            <div className='bg-slate-200 p-4 my-[200px] rounded-lg '>
                <div className='flex justify-end font-bold cursor-pointer' >
                    <span className='font-bold border-4 border-black absolute z-40' onClick={() => setconat(!contact)}>❌</span></div>
                <div>
